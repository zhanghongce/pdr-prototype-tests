...
class PDR(object):
    ...
    def dump_frames(self):
        ...
        print ('#F = %d' % len(self.frames))


    def check_init_failed(self, prop, remove_vars, keep_vars):
        init_cex = self.solve(And(self.frames[0] + [ Not(prop) ] ))
        debug ("[Checking init] F0 and not P")
        if init_cex is not None:
            debug("[Checking init] Property failed at INIT")
            debug("[Checking init] CEX: ", self.print_cube(init_cex))
            return True
        debug ("[Checking init]  F0 and T and not P'")
        init_cex = self.get_bad_state_from_property_invalid_after_trans(prop, 0, remove_vars, keep_vars )
        if init_cex is not None:
            debug("[Checking init] Property failed at F1")
            debug("[Checking init] CEX @F0: ", self.print_cube(init_cex))
            return True
        debug ("[Checking init] Done")
        return False
    ...
    def check_property(self, prop, remove_vars = [], keep_vars = None):
        ...
        debug("[Checking property] Property: %s." % prop)
        ...
        while True:
            ...
            if cube is not None:
                ...
                if not self.recursive_block(cube, len(self.frames)-1, remove_vars, keep_vars ):
                    debug("[Checking property] Bug found at step %d" % (len(self.frames)))
                    return False
                else:
                    debug("[Checking property] Cube blocked '%s'" % self.print_cube(cube))
            else:
                # Checking if the last two frames are equivalent i.e., are inductive
                if self.is_last_two_frames_inductive():
                    debug("[Checking property] The system is safe, frame : %d" % len(self.frames) )
                    return True
                else:
                    debug("[Checking property] Adding frame %d..." % (len(self.frames)))
                    self.frames.append([])
    ...
    def push_lemma_from_the_lowest_frame(self, remove_vars, keep_vars):
        ...
        debug ('[pushes] F%d to F%d' % (start_frame, len(self.frames)-2))
    ...
    def push_lemma_from_frame(self, fidx, remove_vars, keep_vars):
        ...
        if (fidx not in self.cexs_blocked): # else no cex to push
            debug ('  [push_lemma from F%d] <WARN> no cex to push from F%d'%(fidx,fidx))
            pause ()
        ...
        if fidx in self.cexs_blocked:
            ...
            for cexIdx in range(start_cexs_idx,end_cex_idx):
                ...
                debug ('  [push_lemma F%d] cex to try: c%d :'%(fidx, cexIdx), self.print_cube(cex))
                if self.recursive_block(cex, fidx+1, remove_vars, keep_vars):
                    debug ('  [push_lemma F%d] cex is pushed: '%fidx, self.print_cube(cex))
        ...
        while lemmaIdx != end_lemma_idx:
            ...
            if ex is None: # no bad state, lemma is still valid
                ...
                debug ('  [push_lemma F%d] And we add its ITP!'%fidx)
                lemmaIdx += 1 # try next one
            else:
                debug ('  [push_lemma F%d] find cex@F%d : %s' % (fidx, fidx, self.print_cube(ex)))
                if self.recursive_block(ex, fidx, remove_vars, keep_vars ):
                    ...
                    debug ('  [push_lemma F%d] fail due to fact'%fidx , self.print_cube(ex))
                    lemmaIdx+= 1

                    self.dump_frames()
                    debug ('  [push_lemma F%d] Should invoke SyGuS here ... (future work)' % fidx)
        ...
        # how hard to try?
        debug ('  [push_lemma F%d] push lemma finished, press any key to continue'%fidx)
        pause()
    ...
    def recursive_block(self, cube, idx, remove_vars = [], keep_vars = None):
        priorityQueue = []
        debug ('      [block] Try @F%d' % idx, self.print_cube(cube) )
        ...
        while len(priorityQueue) > 0:
            ...
            if model is None:
                ...
                # model is not None
                debug ('      [block] push to queue, F%d' % (fidx-1), self.print_cube(model))
